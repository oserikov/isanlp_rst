...
class SklearnClassifier:
    def __init__(self, model_dir_path):
        ...
        print(os.path.join(self.model_dir_path, 'label_encoder.pkl'))
        file_label_encoder = os.path.join(self.model_dir_path, 'label_encoder.pkl')
        print(file_label_encoder)
        self._label_encoder = pickle.load(open(file_label_encoder, 'rb')) if os.path.isfile(file_label_encoder) else None
        print(dir(self._label_encoder))
    ...
    def _preprocess_features(self, features):
        ...
        res = features.values.astype('float64')
        if self._scaler:
            res = pd.DataFrame(self._scaler.transform(features.values), index=features.index, columns=features.columns)
        print(res.shape)
        return res
